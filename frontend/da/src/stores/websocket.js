...
export const useWebSocketStore = defineStore('websocket', () => {
    ...
    const subscribeToPrivateNotification = () => {
        const topic = '/user/queue/application.lifecycle'
        if(!stompClient.value?.active) return
        if (subscription) subscription.unsubscribe()

        subscription = stompClient.value.subscription(topic, notification => {
            console.log('[Websocket] Notification received:', notification.body)
            lastNotification.value = notification.body
            try{
