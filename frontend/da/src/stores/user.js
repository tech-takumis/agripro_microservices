  ...
  actions: {
    ...
    async updateUser(id, userData) {
      ...
      try {
        ...
        return { success: false, error: this.error, details: error.response?.data };
      } finally {
        this.loading = false;
      }
    },
    ...
    async registerStaff(staffData) {
      ...
      const formattedData = {
        username: staffData.email?.split('@')[0] || staffData.firstname?.toLowerCase(),
    ...
    async assignRole(userId, roleId) {
      ...
      try {
        const response = await axios.post(`/api/v1/users/${userId}/roles/${roleId}`, {}, {
          headers: {
            'Content-Type': 'application/json',
            'Accept': 'application/json',
            'X-Tenant-ID': 'agriculture'
    ...
    async searchUsers(searchTerm, filters = {}) {
      ...
      try {
        ...
        const response = await axios.get(`/api/v1/users/search?${queryParams}`);
